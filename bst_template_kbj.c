 ...
 */
 ...
 //Kim Byeoung Ju
 // this is code from my instructor, i filled some functions and made it work
...
struct Node *createnode(struct BTNode *bst)
{
    struct Node *tmp = (struct Node*)malloc(sizeof(struct Node));
    tmp->next = NULL;
    if(bst->left != NULL ) tmp->key = 1;
    else tmp->key =0;
...
struct Node *isleft(struct BTNode *bst,struct Node *head, struct Node *top)
{
    if(bst == NULL){ return head; }
    else
    {
    struct Node *tmp = createnode(bst);
    if(tmp->key == 1)
    {
        if(head == NULL)
        {
        ...
        else
        {
            ...
            for(cur=head ; cur!=NULL ; cur=cur->next)
            {
               if(cur->key != 0 ) cur->key = cur->key + 1;
            }
            top->next = tmp;
            top = tmp;
        }
...
struct Node *gettop(struct Node *head)
{
...
struct Node *gettopahead(struct Node *head)
{
...
struct Node *freetopnode(struct Node *head)
{
    if(head != NULL)
    {
        struct Node *top,*topahead;
  ...
  //       You may add additional parameter to the function if necessary
{
  ...
  if ( bst != NULL ) {
    ...
    {
        fprintf(fp,"/");
        rlevel = print_BST_2(fp,bst->right,level,head,top);
    }
    ...
    {
        fprintf(fp,"\n");
        head = subandprintblank(fp,head);
        llevel = print_BST_2(fp,bst->left,level,head,top);
    }
    head = freetopnode(head);
    top = gettop(head);
    if(level == 1) fprintf(fp,"\n");
    return rlevel>llevel?rlevel:llevel;
  }
...
struct BTNode **sorttoary(struct BTNode *bst, struct BTNode **aryhead)
{
    static int i = 0;
    if(bst!=NULL)
    {
        if(bst->left != NULL)
        {
            aryhead = sorttoary(bst->left, aryhead);
        }
        *(aryhead+i) = bst;
        i++;
        bst->left = NULL;
        ...
        if(bst->right != NULL)
        {
            aryhead = sorttoary(bst->right, aryhead);
        }
        bst->right = NULL;
    }
    return aryhead;
...
int getnodenum(struct BTNode* bst)
{
    ...
    if(bst!=NULL)
    {
        if(bst->left != NULL)  tmp = getnodenum(bst->left);
        nodenum++;
        if(bst->right != NULL) tmp = getnodenum(bst->right);
    }
    return nodenum;
}
...
int getroot(int numNodes)
{
    int leftnum = numNodes;
    int i = 0;
    int sum = 0;

    while(leftnum > 0)
    {
        if(leftnum <= pow2(2,i)) break;
        else
        {
            leftnum = leftnum - pow2(2,i);
            i++;
        }
    }

    if(leftnum == pow2(2,i)) return numNodes/2;
    else
    {
        if(pow2(2,i)/2 <= leftnum )
        {
            for(int a=0; a<=i;a++)
            {
                sum = sum + pow2(2,a)/2;
...
// using quick sort
/////////////////////////////////////////////////////////////
struct BTNode *extracthead(struct BTNode *lbst)
{
    struct BTNode *ncur;
    ncur = lbst->left;
    lbst->left = NULL;
...
struct BTNode *addtotail(struct BTNode *nhead, struct BTNode *cur)
{
    struct BTNode *tail = nhead;

    if(nhead == NULL && tail == NULL )
    {
        nhead = tail = cur;
    }
    else
...
struct BTNode *partition(struct BTNode *lbst)
{
    struct BTNode *qroot,*nroot,*nnroot,*ln,*rn;
    ln = rn = NULL;
    qroot = lbst;
    nroot = extracthead(qroot);

    while(nroot != NULL)
    {
        nnroot = extracthead(nroot);
        if(comparekey(nroot,qroot)==-1||comparekey(nroot,qroot)==0)
        {
            ln = addtotail(ln,nroot);
        }
        else rn = addtotail(rn,nroot);
        nroot = nnroot;
    }
    qroot->left = ln;
    qroot->right = rn;
...
int main()
{
  ...
  struct Node *head,*top; // for print 2
  head = top = NULL;
  ...
  if ( root != NULL ) {
    ...
    fprintf(fp,"BST height %d\n",lev);
    fprintf(fp,"=====================================\n");
    lev = print_BST_2(fp,bst1,0,head,top);
    fprintf(fp,"BST height %d\n",lev);
    fprintf(fp,"=====================================\n");
    ...
    lev = print_BST_2(fp,bst2,0,head,top);
